...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for ele in table.find_all('tr'):
            elements = []
            for name in ele.find_all('td'):
                elements.append(name.text)
            table_elements.append(elements)
        ...
        for i in range(1,len(table_elements)):
            ...
            if (table_elements[i][1] == 'TOTAL'):
                continue
            
            dict = {}
            dict["courier"] = "FL"
            dict["airport"] = "ATL"
            dict["year"] = int(table_elements[i][0])
            dict["month"] = int(table_elements[i][1])
            dom = table_elements[i][2]
            inter = table_elements[i][3]
            if (inter ==  u'\xa0'):
                dict["flights"] = {"international" : " ",
                                   "domestic" : int(dom.replace(',',''))}
            dict["flights"]= {"international" : int(inter.replace(',','')),
                              "domestic" : int(dom.replace(',',''))}
